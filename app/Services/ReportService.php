...
class ReportService
{
    ...
    public function exportCsv(array $filters): StreamedResponse
    {
        ...
        return new StreamedResponse(function () use ($data) {
            ...
            // Header row
            fputcsv($handle, [
                'Tanggal', 'No. Transaksi', 'Nama', 'Kategori', 'Tipe',
                'Masuk', 'Keluar', 'Saldo', 'Jamaah/Pihak', 'Bank/Kas Asal', 'Bank/Kas Tujuan',
            ]);
    ...
    {
        if (! empty($filters['tanggal_mulai']) && ! empty($filters['tanggal_akhir'])) {
            return Carbon::parse($filters['tanggal_mulai'])->format('d/m/Y').' - '.Carbon::parse($filters['tanggal_akhir'])->format('d/m/Y');
        }
        ...
        return match ($periode) {
            'mingguan' => 'Pekan '.$now->weekOfYear.' ('.$now->copy()->startOfWeek(Carbon::MONDAY)->format('d/m/Y').' - '.$now->copy()->endOfWeek(Carbon::SUNDAY)->format('d/m/Y').')',
