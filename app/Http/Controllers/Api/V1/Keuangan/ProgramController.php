...
class ProgramController extends Controller
{
    ...
    public function physicalBalances($id): JsonResponse
    {
        ...
        $transactions = Transaction::where('program_id', $id)
        ...
        foreach ($transactions as $trx) {
            if ($trx->tipe === 'pemasukan') {
                ...
                if (! isset($balances[$kasAsal])) {
                    $balances[$kasAsal] = 0;
                }
                if (! isset($balances[$kasTujuan])) {
                    $balances[$kasTujuan] = 0;
                }
        ...
        $bankKas = BankKas::whereIn('id', $bankKasIds)->get()->keyBy('id');
    ...
    public function rollover(Request $request, $id): JsonResponse
    {
        ...
        DB::transaction(function () use ($sourceProgram, $targetProgram, $targetProgramId, $validated, $request) {
            ...
            foreach ($validated['sources'] as $source) {
                $kas = BankKas::find($source['bank_kas_id']);
